...
### Resources (`resources/`)
- `classes/` -- 51 FighterData .tres files (5 base + 14 T1 + 30 T2 + 2 royal)
...
- `spriteframes/` -- 344 SpriteFrames .tres files (in `assets/art/sprites/spriteframes/`, tracked in git); 273 chibi 3-direction sprites (54 player class males + 54 female variants via palette swaps + ~120 enemy sprites) + 71 existing 4-direction/sheet sprites; generated by `generate_all_sprites.py`
...
- `batch_palette_swaps.py` -- Batch driver for all 60 class sprite palette swaps (Squire/Mage/Entertainer/Scholar trees); defines source→output mappings with hue targets per class
...
- `set_sprite_ids.py` -- Python tool to set sprite_id and sprite_id_female on all 54 class and 78 enemy .tres files; classes mapped to 54 unique Chibi sprites from 38 packs; 15 gender variant mappings with palette-matched recolors; 78 enemies mapped to unique Chibi sprites from 40 packs; prints coverage report
...
| 13 | COMPLETE | Art integration — 231 CraftPix packs downloaded (96 RPG top-down + 98 Chibi + 37 Tiny Fantasy); direction synthesis pipeline; all 54 player classes mapped with male + female variants (60 palette swaps for gender/elemental recolors); 78 enemies mapped; 344 SpriteFrames generated |
