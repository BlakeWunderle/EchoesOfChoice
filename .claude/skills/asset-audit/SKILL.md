...
Manages the split between the Godot project (`EchoesOfChoice/assets/`) and the asset library (`assets_library/`). Verifies no referenced assets are missing, manages music track selection, and maintains `.gdignore` exclusions.
...
|----------|---------|------|
| `EchoesOfChoice/assets/` | Runtime assets (sprites, music, tilesets, audio) | ~2-3 GB |
...
python EchoesOfChoice/tools/asset_audit.py

# Place .gdignore files in unused directories
python EchoesOfChoice/tools/asset_audit.py --fix

# Size report only
python EchoesOfChoice/tools/asset_audit.py --sizes
```
...
python EchoesOfChoice/tools/curate_music.py

# Change tracks per context
python EchoesOfChoice/tools/curate_music.py --per-context 10

# Sync from existing manifest (restore selections after fresh clone)
python EchoesOfChoice/tools/curate_music.py --sync

# List current track selections and check for missing files
python EchoesOfChoice/tools/curate_music.py --list
```
...
3. Run `python EchoesOfChoice/tools/asset_audit.py` to verify
...
2. Run `python EchoesOfChoice/tools/curate_music.py --sync` to move files accordingly
3. Commit the updated manifest

### Before export

1. Run `python EchoesOfChoice/tools/asset_audit.py` — should pass with 0 warnings
2. Run `python EchoesOfChoice/tools/asset_audit.py --fix` to ensure all `.gdignore` files are placed
3. Build: `"C:/Users/blake/AppData/Local/Microsoft/WinGet/Packages/GodotEngine.GodotEngine_Microsoft.Winget.Source_8wekyb3d8bbwe/Godot_v4.6.1-stable_win64_console.exe" --path EchoesOfChoice --headless --quit`

### After fresh clone / new machine

1. Download asset packs (or copy `assets_library/` from backup)
2. Run `python EchoesOfChoice/tools/curate_music.py --sync` to restore music selections
3. Run `python EchoesOfChoice/tools/asset_audit.py --fix` to place `.gdignore` files
