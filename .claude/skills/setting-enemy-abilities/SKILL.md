...
**When the enemy is for a specific battle:** Before creating a new .tres, check [making-battle-configs reference](.cursor/skills/making-battle-configs/reference.md) “C# battle → tactical roster” — use the exact C# class name and mapping (e.g. Harlequin → harlequin.tres, Chanteuse → chanteuse.tres). Create that .tres with stats/abilities from C# `CharacterClasses/Enemies/<Name>.cs` and `Abilities/Enemy/*.cs` so the battle config can use it and the encounter makes sense (e.g. circus = performers only).
...
- [making-battle-configs](.cursor/skills/making-battle-configs/SKILL.md): when creating new enemies for a battle, use this skill to set their abilities. See making-battle-configs [reference.md](.cursor/skills/making-battle-configs/reference.md) “C# battle → tactical roster” for which C# enemies map to which tactical .tres (e.g. cemetery: 2 zombies + 2 ghosts + 1 wraith → bone_sentry, shade, wraith).
