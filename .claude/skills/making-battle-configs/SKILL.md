...
- **Progression**: Battles should feel like the game is advancing. Use the node's **progression** from `MapData.get_node(battle_id).progression` (0–7 in map_data.gd).
  ...
  - **Enemy type by stage**: Pick enemy types that match the **stage** of the game so later battles feel harder and more advanced, not the same as the start.
    ...
    - **Late (progression 6–7)**: Elite or peak — guard elites, shadow elites (gloom_stalker, shadow_hound, dusk_moth), hellion, fiendling, seraph, necromancer, psion, shaman, warlock, earth_elemental, or other "boss-tier" types. Lead unit should feel like the climax of that stretch. Don't use early-game-only types (e.g. lone thugs, street_tough, goblin) as the main roster for return-city or elemental gates — those are prog-0 enemies. **Elemental shrines (prog 7)**: use mono-element composition — 1 powerful named lead (lvl 8) + 4 lesser of the **same** elemental type (lvl 7). No mixing elements within one shrine. Four shrines cover fire, water, air, and earth — no element repeats across shrines.
