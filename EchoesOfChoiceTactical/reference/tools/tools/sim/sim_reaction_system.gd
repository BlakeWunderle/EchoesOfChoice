## Adapted ReactionSystem for headless simulation.
## Removes SFXManager calls and Unit type checks — uses preloaded SimUnit instead.
class_name SimReactionSystem extends RefCounted
...
func check_opportunity_attacks(moving_unit, from_pos: Vector2i, to_pos: Vector2i) -> Array[Dictionary]:
	...
	for dir in Grid.DIRECTIONS:
		...
		results.append({
			"reactor": occupant, "target": moving_unit,
			"type": Enums.ReactionType.OPPORTUNITY_ATTACK,
			"damage": damage if not dodged else 0,
			"dodged": dodged, "critted": critted,
		})
	return results


func check_flanking_strikes(attacker, target) -> Array[Dictionary]:
	var results: Array[Dictionary] = []
	for dir in Grid.DIRECTIONS:
		var adj: Vector2i = target.grid_position + dir
		var occupant = grid.get_occupant(adj)
		...
		if not Combat.roll_dodge(target.dodge_chance):
			...
			occupant.use_reaction()
			results.append({
				"reactor": occupant, "target": target,
				"type": Enums.ReactionType.FLANKING_STRIKE,
				"damage": damage, "dodged": false,
			})
		else:
			occupant.use_reaction()
			results.append({
...
func check_snap_shot(approaching_unit, from_pos: Vector2i, to_pos: Vector2i) -> Array[Dictionary]:
	var results: Array[Dictionary] = []
	for dir in Grid.DIRECTIONS:
		var adj := to_pos + dir
		var occupant = grid.get_occupant(adj)
		...
		if not Combat.roll_dodge(approaching_unit.dodge_chance):
			...
			occupant.use_reaction()
			results.append({
				"reactor": occupant, "target": approaching_unit,
...
func check_reactive_heal(damaged_unit, _damage_amount: int) -> Array[Dictionary]:
	var results: Array[Dictionary] = []
	var allies := _get_allies_in_range(damaged_unit, 3)
	...
	for ally in allies:
		...
		ally.use_reaction()
		results.append({
			"reactor": ally, "target": damaged_unit,
...
func check_damage_mitigation(damaged_unit, incoming_damage: int) -> Dictionary:
	var allies := _get_allies_in_range(damaged_unit, 3)
	...
	for ally in allies:
		...
		return {
			...
			"original_damage": incoming_damage, "mitigated_damage": mitigated,
			"reduction": incoming_damage - mitigated, "active": true,
		}
	return {"active": false}


func check_bodyguard(damaged_unit, incoming_damage: int) -> Dictionary:
	for dir in Grid.DIRECTIONS:
		var adj: Vector2i = damaged_unit.grid_position + dir
		var occupant = grid.get_occupant(adj)
		...
		occupant.use_reaction()
		return {
			"reactor": occupant, "target": damaged_unit,
...
func process_defensive_reactions(target, raw_damage: int) -> Dictionary:
...
func _is_valid_reactor(unit, against, reaction_type: Enums.ReactionType) -> bool:
	if unit == null or not unit is _SimUnit:
		return false
...
func _get_allies_in_range(unit, support_range: int) -> Array:
	var allies: Array = []
	for x in range(-support_range, support_range + 1):
		for y in range(-support_range, support_range + 1):
			...
			if x == 0 and y == 0:
				continue
			var pos: Vector2i = unit.grid_position + Vector2i(x, y)
			var occupant = grid.get_occupant(pos)
			if occupant != null and occupant is _SimUnit and occupant != unit:
