using EchoesOfChoice.CharacterClasses.Common;
using EchoesOfChoice.CharacterClasses.Abilities;
using System.Collections.Generic;

namespace EchoesOfChoice.CharacterClasses.Mage
{
    public class Pyromancer : BaseFighter
    {
        public Pyromancer()
        {
            Abilities = new List<Ability>() { new FireBall(), new Inferno(), new Ignite() };
            CharacterType = "Pyromancer";
            CritChance = 2;
            CritDamage = 2;
            DodgeChance = 3;
        }

        public Pyromancer(BaseFighter fighter) : base(fighter) { }

        public override BaseFighter Clone()
        {
            return new Pyromancer(this);
        }

        protected override void ApplyUpgradeBonuses()
        {
            MagicAttack += 8;
            Speed += 3;
        }

        public override void IncreaseLevel()
        {
            Level += 1;
            var healthIncrease = random.Next(4, 7);
            Health += healthIncrease;
            MaxHealth += healthIncrease;
            var manaIncrease = random.Next(2, 5);
            Mana += manaIncrease;
            MaxMana += manaIncrease;
            PhysicalAttack += random.Next(1, 3);
            PhysicalDefense += random.Next(1, 2);
            MagicAttack += random.Next(8, 11);
            MagicDefense += random.Next(1, 2);
            Speed += random.Next(2, 5);
        }

        public override BaseFighter UpgradeClass(UpgradeItemEnum upgradeItem)
        {
            throw new System.NotImplementedException();
        }
    }
}
