...
echo.
echo [2/3] macOS (works on both Intel and M1/M2/M3 via Rosetta)...
dotnet publish %PROJECT% -c %CONFIG% -r osx-x64 -o "publish\mac"
if %errorlevel% neq 0 goto error

echo.
echo [3/3] Linux (x64)...
dotnet publish %PROJECT% -c %CONFIG% -r linux-x64 -o "publish\linux"
if %errorlevel% neq 0 goto error
...
echo   publish\linux\    - send to Linux friends
echo.
...
echo.
echo NOTE for Linux friends: After receiving the file they need to
echo run this once in Terminal to allow it to execute:
echo   chmod +x "Echoes of Choice"
echo Then run it with:
echo   ./"Echoes of Choice"
echo ================================================
pause
exit /b 0
